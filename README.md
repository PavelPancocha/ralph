...
`--checkout-mode worktree` is the default. `--checkout-mode root` is the fallback for repos whose Docker or compose setup cannot run correctly from a Ralph worktree. Root mode reuses the repo root itself as the active checkout for each selected spec, supports the same selection shapes as ordinary runs (including `--to`), snapshots the selected specs before branch switching so later spec reads do not depend on the currently checked-out repo branch, requires a clean non-detached repo checkout before each spec setup, leaves the repo on the last processed branch, and exposes `/var/run` to implementer and reviewer sandboxes when the spec's verification commands reference Docker. The one exception to the clean-tree rule is an interrupted same-spec rerun in root mode: Ralph audits the dirty checkout against saved state, artifacts, event logs, and saved implementer session evidence. A passing audit emits a `recovery` progress event and re-enters the implementer stage with recovery context. A failing audit for the same interrupted spec writes recovery audit/stash artifacts, stashes the dirty checkout with `git stash push -u`, and restarts from a clean checkout. If the dirty repo cannot be linked to the same spec, Ralph still fails fast instead of touching unrelated work.
